...
Shader "ASE/GrassAnimated"
{
	Properties
	{
		...
		_WindStrength("WindStrength", Range( 0 , 1)) = 0.6
		_WindSpeed("WindSpeed", Range( 0 , 1)) = 0.79
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			float2 panner26 = ( 1.71 * _Time.y * ( _Time.x * float2( 0.001,0 ) ) + ase_worldPos.xy);
			float simplePerlin2D30 = snoise( panner26*_WindSpeed );
			simplePerlin2D30 = simplePerlin2D30*0.5 + 0.5;
			float4 appendResult39 = (float4(( ( ( simplePerlin2D30 - 0.5 ) * _WindStrength ) + ase_worldPos.x ) , ase_worldPos.y , ase_worldPos.z , 0.0));
			float4 ase_screenPos = ComputeScreenPos( UnityObjectToClipPos( v.vertex ) );
			float4 ase_screenPosNorm = ase_screenPos / ase_screenPos.w;
			ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
			float3 worldToObj40 = mul( unity_WorldToObject, float4( ( appendResult39 - ase_screenPosNorm ).xyz, 1 ) ).xyz;
			float3 Sway41 = worldToObj40;
			v.vertex.xyz += Sway41;
		}
...
/*ASEBEGIN
Version=17101
76;35;1374;640;-711.1673;143.9771;1.659999;True;False
...
ASEEND*/
//CHKSM=FAA12C695057B1EB7FC8C72B2F624088F2291212